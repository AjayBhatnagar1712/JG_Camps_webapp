...
export default function PlannerModal({ open, onClose }: Props) {
  ...
  // form fields
  const [arrive, setArrive] = useState("");     // yyyy-mm-dd
  ...
  function onArriveChange(v: string) {
    ...
    if (depart && v && new Date(depart) <= new Date(v)) {
      const d = new Date(v);
      d.setDate(d.getDate() + 1); // bump departure to next day
  ...
  async function generate() {
    ...
    const system = `You are Travel Assistant for JG Camps & Resorts.
  ...
  return (
    <div className="fixed inset-0 z-[60]">
      ...
      <div className="absolute inset-0 grid place-items-center p-4">
        <div className="w-full max-w-3xl rounded-3xl bg-white/90 dark:bg-neutral-900/90 border border-border shadow-2xl backdrop-blur overflow-hidden">
          {/* header */}
            ...
            <div className="grid gap-3">
              ...
              {datesInvalid && (
                <p className="text-xs text-red-600 -mt-1">
                  Departure must be after Arrival. I’ve nudged it automatically if needed.
                </p>
              )}
              ...
              <textarea
                className="rounded-xl border border-border px-3 py-2 text-sm min-h-[84px]"
                placeholder="Anything specific? (e.g., high passes, ropeway, café crawls, sunrise points)"
                value={notes}
              ...
              <button
                ...
                className="mt-1 inline-flex items-center justify-center gap-2 rounded-2xl bg-primary text-primary-foreground px-4 py-2.5 font-semibold shadow hover:opacity-90 disabled:opacity-50"
                aria-disabled={!canSubmit || state === "loading"}
              >
                {state === "loading" ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin" /> Creating itinerary…
                  </>
                ) : (
                  <>
                    <Sparkles className="h-4 w-4" /> Build Itinerary <ChevronRight className="h-4 w-4" />
                  </>
                )}
              </button>
            ...
            <div className="rounded-2xl border border-border bg-card p-4 shadow-sm min-h-[260px]">
              ...
              {state === "loading" && (
                <div className="space-y-3 animate-pulse">
                  <div className="h-5 w-2/5 rounded bg-muted/60" />
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {arrive && depart && (
                      <span className="inline-flex items-center gap-2 rounded-full border border-border px-3 py-1 text-xs">
                        <Calendar className="h-4 w-4" /> {new Date(arrive).toLocaleDateString()} → {new Date(depart).toLocaleDateString()} {nights ? `• ${nights} nights` : ""}
                      </span>
                    )}
                    {destinations.length > 0 && (
                      <span className="inline-flex items-center gap-2 rounded-full border border-border px-3 py-1 text-xs">
                        <MapPin className="h-4 w-4" /> {destinations.join(" • ")}
                      </span>
                    )}
                    {budget && (
                      <span className="inline-flex items-center gap-2 rounded-full border border-border px-3 py-1 text-xs">
                  ...
                  {days.map((d, i) => (
                    <div key={i} className="rounded-xl border border-border overflow-hidden">
                      ...
                      <div className="p-3 prose prose-sm max-w-none text-foreground [&_ul]:list-disc [&_ul]:ml-5">
                        {d.body.split("\n").map((ln, idx) => (
                          <p key={idx} className={ln.startsWith("•") || ln.startsWith("-") ? "ml-3" : ""}>
                            {ln}
                          </p>
                        ))}
                      </div>
                  ...
                  <div className="rounded-xl bg-primary text-primary-foreground p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <a href="tel:+918076874156" className="inline-flex items-center gap-2 rounded-lg bg-white text-foreground px-3 py-2 font-semibold shadow hover:bg-muted">
                        <Phone className="h-4 w-4" /> 8076874156
                      </a>
                      <a href="tel:+918595167227" className="inline-flex items-center gap-2 rounded-lg bg-white text-foreground px-3 py-2 font-semibold shadow hover:bg-muted">
                        <Phone className="h-4 w-4" /> 8595167227
                      </a>
                      <a href="mailto:jgadven@gmail.com?subject=Itinerary%20help" className="inline-flex items-center gap-2 rounded-lg border border-white/40 px-3 py-2 font-semibold hover:bg-white/10">
                        <Mail className="h-4 w-4" /> jgadven@gmail.com
                      </a>
                    </div>
                  </div>
                </div>
          ...
          {/* footer row with close */}
          <div className="px-6 pb-5">
            <button
              onClick={onClose}
              className="w-full sm:w-auto rounded-xl border border-border px-4 py-2 text-sm hover:bg-muted"
            >
              Close
            </button>
          </div>
