...
const POPULAR_SPOTS: Record<(typeof EAST_STATES)[number], string[]> = {
  Bihar: ["Patna", "Nalanda", "Bodh Gaya", "Vaishali"],
  Jharkhand: ["Ranchi", "Dumka", "Hazaribagh", "Netarhat"],
  Odisha: ["Puri", "Konark", "Bhubaneswar", "Chilika Lake"],
  "West Bengal": ["Kolkata", "Darjeeling", "Sundarbans", "Shantiniketan"],
  "Andaman & Nicobar Islands": ["Port Blair", "Havelock Island", "Neil Island", "Ross Island"],
};

export default function EastIndiaPlanTrip() {
  ...
  async function buildAIItinerary(): Promise<string> {
    ...
    const stateList = selectedStates.join(", ") || "East Indian states";
    const destinationList = allSpots.join(", ") || "East Indian highlights";

    const systemPrompt = `You are a professional travel planner for **JG Camps & Resorts**, specializing in East India.
Rules:
...
- Mention approximate travel times, 2–3 activity highlights for each day, and local food suggestions.
- Include short cost guidance (Economy / Mid / Premium) but avoid naming properties.
- Keep the tone concise, helpful and travel-savvy.
- End with a short summary encouraging users to contact JG Camps & Resorts for bookings.`;

    const userPrompt = `Plan a ${days}-day ${tripType || "East India"} itinerary.
States: ${stateList}
Spots / regions: ${destinationList}
Budget: ${budget || "flexible"}
Make it balanced and traveler-friendly.`;
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10">
      <div className="text-center mb-10">
        <h1 className="text-4xl font-extrabold text-gray-900">Build Your East India Itinerary</h1>
        <p className="text-lg text-gray-600 mt-3">
          AI-powered itinerary builder for Bihar, Odisha, West Bengal, Jharkhand & the Andamans — curated by JG Camps & Resorts.
          No hotel names will be shown — contact us to book stays and packages.
        </p>
      </div>
      ...
      <div className="max-w-2xl mx-auto bg-white p-6 rounded-2xl shadow-lg">
        {!showItinerary ? (
          ...
          >
            ...
            {currentStep === 1 && (
              <Step title="What kind of trip?">
                {["Adventure", "Spiritual", "Family", "Wellness", "Cultural", "Leisure"].map((t) => (
            ...
            {currentStep === 5 && (
              <div>
                ...
                <p className="text-xs text-gray-500 mt-3">AI will generate your custom East India itinerary.</p>
              </div>
            )}
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
            <h2 className="text-2xl font-bold mb-4 text-center">Your East India Itinerary</h2>
