...
/**
 * North East India planner (wizard)
 * Path to save: app/india/northeast/plan/page.tsx
 *
 * Uses your /api/gemini endpoint to generate itineraries.
 * Rules: do NOT mention hotel/resort names. Provide contact details instead.
 */
...
const POPULAR_SPOTS: Record<(typeof NORTHEAST_STATES)[number], string[]> = {
  "Arunachal Pradesh": ["Tawang", "Ziro Valley", "Bomdila"],
  Assam: ["Kaziranga", "Majuli", "Guwahati", "Sivasagar"],
  Manipur: ["Imphal", "Loktak Lake", "Keibul Lamjao"],
  Meghalaya: ["Shillong", "Cherrapunji", "Dawki", "Mawlynnong"],
  Mizoram: ["Aizawl", "Reiek", "Phawngpui (Blue Mountain)"],
  Nagaland: ["Kohima", "Dzukou Valley", "Mon Village"],
  Sikkim: ["Gangtok", "Tsomgo Lake", "Pelling", "Yumthang Valley"],
  Tripura: ["Agartala", "Unakoti", "Neermahal"],
};

export default function NorthEastPlanTrip() {
  ...
  const spotsFromSelectedStates = selectedStates.flatMap((st) => POPULAR_SPOTS[st as keyof typeof POPULAR_SPOTS] || []);

  /** Gemini AI Integration for North East India */
  async function buildAIItinerary(): Promise<string> {
    ...
    const stateList = selectedStates.join(", ") || "North East states";
    const destinationList = allSpots.join(", ") || "North East highlights";

    const systemPrompt = `You are a professional travel planner for **JG Camps & Resorts**, specializing in North East India.
Rules:
- DO NOT mention or recommend any specific hotel, resort, Airbnb, or stay property by name.
...
- Mention approximate travel times, 2–3 activity highlights per day, and 1–2 food/cafe suggestions (no property names).
- Suggest 2–3 stay tiers (Economy / Mid / Premium) with rough INR ranges.
- End with a one-line summary encouraging users to contact JG Camps & Resorts for bookings.
- Keep the tone concise, informative and user-friendly.`;

    const userPrompt = `Plan a ${days}-day ${tripType || "North East India"} itinerary.
States: ${stateList}
Spots / regions: ${destinationList}
Budget: ${budget || "flexible"}
Balance nature, culture, and comfortable travel pacing.`;
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10">
      <div className="text-center mb-10">
        <h1 className="text-4xl font-extrabold text-gray-900">Build Your North East India Itinerary</h1>
        <p className="text-lg text-gray-600 mt-3">
          Generate an AI-powered plan for Arunachal, Assam, Meghalaya, Sikkim and more. No hotel names — just routes, highlights & our contact for bookings.
        </p>
      </div>
      ...
      <div className="max-w-2xl mx-auto bg-white p-6 rounded-2xl shadow-lg">
        {!showItinerary ? (
          <motion.div key={currentStep} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
            {/* Step 0: Duration */}
            {currentStep === 0 && (
            ...
            {currentStep === 1 && (
              <Step title="What kind of trip?">
                {["Adventure", "Cultural", "Family", "Spiritual", "Wellness", "Leisure"].map((t) => (
            ...
            {currentStep === 5 && (
              <div>
                ...
                <ul className="text-gray-700 space-y-2">
                  ...
                  <li>
                    <strong>Places:</strong> {selectedSpots.join(", ") || customSpot || "—"}
                  </li>
                  <li>
                    <strong>Budget:</strong> {budget || "—"}
                  </li>
                </ul>
                <p className="text-xs text-gray-500 mt-3">AI will generate a detailed itinerary following these preferences.</p>
              </div>
            )}
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
            <h2 className="text-2xl font-bold mb-4 text-center">Your North East India Itinerary</h2>
